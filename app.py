...
def query_api(food_name, disease):
    ...
    for response in a.generations:
        # 假设API返回的是一个包含食品名称、图片链接和作用的字典
        return [{"name": food_name, "image": "", "effect": response[0].text}]
...
def process_image_links(data):
    ...
    elif isinstance(data, list):  # 处理API返回的列表
        return data
    elif isinstance(data, str):  # 处理单个字符串
        return [{"name": "", "image": "", "effect": data}]
    else:
        return data
...
def search_food():
    ...
    WHERE name LIKE %s
"""
    cursor.execute(search_query, ('%' + query + '%',))  # 使用通配符包裹查询字符串
